...
lemmatize <- function(text, id) {
  ...
  tag_results <- tag_results[pos %in% c('ADJ', 'ADV', 'NOM') | stringi::stri_detect_regex(str = pos, pattern = '^VER')]
  tag_results <- tag_results[!lemma %in% c('être', "avoir", "être/suivre")]
  tag_results <- tag_results[, .(count = .N), by = lemma]
